...
void InputMgr::Init()
{
    ...
    infoH.positives.push_back(sf::Keyboard::D);     //1을 입력 받기 위함
    infoH.positives.push_back(sf::Keyboard::Right);

    infoH.negatives.push_back(sf::Keyboard::A);     //-1을 입력 받기 위함
...
float InputMgr::GetAxisRaw(Axis axis)         // 입력에 따라 -1 0 1을 반환
{                                             // positives : 양의 입력 1 반환
    auto findInfo = axisInfoMap.find(axis);   // negatives : 음의 입력 -1 반환
